...
const MemberExpensesPieChart = ({ totalExpensePerMember }: Props) => {
  const dynamicChartConfig = totalExpensePerMember.reduce(
    (acc, member, index) => {
  ...
  const chartData = totalExpensePerMember.map((item, index) => ({
  ...
  return (
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-1">
        {totalExpensePerMember.length > 0 ? (
          <>
            <ChartContainer
            ...
            <div className="space-y-2">
              {totalExpensePerMember.map((member) => (
                <PercentageItem
                  key={member.memberId}
...
export default MemberExpensesPieChart;
